...
| S2 | A slot is identified by `(inventory_id, slot_x, slot_y)`. `Flask1` holds `slot_x` 0 (life flask) and 1 (mana flask); `Trinket1`, the charm belt, holds 2, 3 and 4. `slot_y` is 0 everywhere. An entry at a position outside this curated vocabulary (e.g. `Flask1` at `slot_x` 2, or `slot_y` ≠ 0) stays invisible in the editor and cannot be cleared there, though it survives export. | data | `DocumentEditor` (addresses entries by `(inventory_id, slot_x)`, y = 0) and the one `position` form field `<inventory_id>@<slot_x>` parsed in `CommandFactory` |
