...
| T5 | A declared keystone jewel lets a non-keystone node within 1000 units of its keystone be allocated disconnected. Its keystone need not be allocated. It is a separate mechanism from T4 and does not use `keystonesInRadius`. | owner 0.5.5 (proofs 11, 12); the 1000-unit reach is assumed (proof 12) | Today it uses T4's list. Slice B2 (plan amendment) changes that to `PassiveGraph::jewelCovers()`. |
